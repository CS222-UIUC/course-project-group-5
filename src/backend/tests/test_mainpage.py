...
class TestMainPage:
    ...
    def insert_reviews(self):
        ...
        self.cursor.execute(
            ...
            VALUES (?, ?, 2022-10-07, 'Pretty good', TRUE)", (sherman_id, minh_phan_id)
        )
        self.cursor.execute(
            "INSERT INTO Ratings (apt_id, user_id, date_of_rating, comment, vote) \
            VALUES (?, ?, 2022-10-08, 'Bruh this sucks', FALSE)", (sherman_id, minh_id)
        )
        self.cursor.execute(
            "INSERT INTO Ratings (apt_id, user_id, date_of_rating, comment, vote) \
            VALUES (?, ?, 2022-10-09, 'Decent', TRUE)", (sherman_id, big_finger_id)
        )
        self.cursor.execute(
            "INSERT INTO Ratings (apt_id, user_id, date_of_rating, comment, vote) \
            VALUES (?, ?, 2022-10-10, 'Decent hall', TRUE)", (far_id, big_finger_id)
        )
        self.cursor.execute(
            "INSERT INTO Ratings (apt_id, user_id, date_of_rating, comment, vote) \
            VALUES (?, ?, 2022-10-11, 'Why', FALSE)", (par_id, big_finger_id)
        )

    def clean_up_users(self):
        """Delete users inserted during test"""
        self.cursor.execute(
            "DELETE FROM Users WHERE username = 'Minh Phan'"
        )
        self.cursor.execute(
            "DELETE FROM Users WHERE username = 'Minh'"
        )
        self.cursor.execute(
            "DELETE FROM Users WHERE username = 'Big_finger'"
        )

    def clean_up_apartments(self):
        """Delete apartments inserted during test"""
        self.cursor.execute(
            "DELETE FROM Apartments WHERE apt_name = 'Sherman'"
        )
        self.cursor.execute(
            "DELETE FROM Apartments WHERE apt_name = 'FAR'"
        )
        self.cursor.execute(
            "DELETE FROM Apartments WHERE apt_name = 'Lincoln'"
        )
        self.cursor.execute(
            "DELETE FROM Apartments WHERE apt_name = 'PAR'"
        )
    ...
    def clean_up_reviews(self):
        ...
        self.cursor.execute(
            "DELETE FROM Ratings WHERE apt_id = ?",
            (sherman_id,)
        )
        self.cursor.execute(
            "DELETE FROM Ratings WHERE apt_id = ?",
    ...
    def clean_up_pics(self):
        ...
        self.cursor.execute(
            "DELETE FROM AptPics WHERE apt_id = ?", (sherman_id,)
        )
    ...
    def test_apartments_default(self):
        ...
        sample_apts_default.append(Apt("Lincoln", "1005 S Lincoln Ave", 0, (5000, 6000)))
    ...
    def test_get_apartments_reviews(self):
        ...
        sample_apts_review = []
        sample_apts_review.append(Review("Minh Phan", "2022-10-07", "Pretty good", True))
        sample_apts_review.append(Review("Minh", "2022-10-08", "Bruh this sucks", False))
        sample_apts_review.append(Review("Big_finger", "2022-10-09", "Decent", True))
        self.initialize_all()
        res = self.main_page.get_apartments_reviews("Sherman")
        self.clean_all()
        assert self.sample_apts_review == res